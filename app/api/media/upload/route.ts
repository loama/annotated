...
export async function POST(request: NextRequest) {
  ...
  if (!user) return NextResponse.json({ error: "Sign in with Google to upload commentary" }, { status: 401 });
  ...
  try {
    return NextResponse.json(await transcodeUpload(file, kind, user.id), { status: 201 });
  } catch (reason) {
