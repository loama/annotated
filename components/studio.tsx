    ...
    try {
      ...
      if (youtubeSource && fromExtension && !mediaUrl) {
        ...
        try {
          ...
          const response = await fetch("/api/media/upload", { method: "POST", headers: sessionToken ? { "x-annotated-session": sessionToken } : undefined, body: form });
      ...
      if (draft.sourceType !== "article" && !youtubeSource && !mediaUrl) {
        const response = await fetch("/api/media/process", { method: "POST", headers: { "content-type": "application/json", ...(sessionToken ? { "x-annotated-session": sessionToken } : {}) }, body: JSON.stringify({ sourceUrl: draft.sourceUrl, mediaUrl: draft.mediaUrl, sourceType: draft.sourceType, startSeconds: draft.startSeconds, endSeconds: draft.endSeconds }) });
      ...
      const response = await fetch("/api/annotations", { method: "POST", headers: { "content-type": "application/json", ...(sessionToken ? { "x-annotated-session": sessionToken } : {}) }, body: JSON.stringify(annotation) });
