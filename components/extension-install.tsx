    ...
    <AppShell>
      <section className="mx-auto grid max-w-[1400px] grid-cols-1 items-end gap-12 px-4 pb-16 pt-28 md:grid-cols-12 md:px-7 md:pb-20 md:pt-32">
        ...
        <div className="flex flex-col items-start md:col-span-5 md:pb-2">
          ...
          <p className="mt-4 font-mono text-[0.58rem] uppercase tracking-[0.12em] text-[var(--ink-muted)]">{mobileDevice ? "The extension requires Chrome on desktop" : "Chrome 116+ on desktop · Native side panel"}</p>
        </div>
      </section>
            ...
            <div className="divide-y divide-[var(--line)] border-y border-[var(--line)]">
              {steps.map((step) => (
                <div key={step.number} className="grid grid-cols-[3rem_1fr] gap-4 py-7 md:grid-cols-[4rem_1fr_auto] md:items-center md:gap-6">
                  ...
                  {step.number === "01" && (
                    <a href="/annotated-chrome-extension-v1.0.4.zip" download className="pressable col-start-2 mt-3 flex w-fit items-center gap-2 rounded-full bg-[var(--ink)] px-4 py-3 text-xs font-semibold text-white md:col-start-auto md:mt-0">
        ...
        <div className="paper-shell">
          <div className="paper-core grid gap-10 p-7 md:grid-cols-[1fr_auto] md:items-center md:p-11">
            <div>
              ...
              <p className="mt-5 max-w-[58ch] text-sm leading-relaxed text-[var(--ink-muted)]">It uses the active tab to read the URL, selected text, and current media time. Tab capture starts only after you press the extension-owned Start recording button, then records the fully visible YouTube player and audio for the selected moment. It can read only the Annotated session cookie so your side panel recognizes an existing Annotated sign-in. It cannot access YouTube cookies, read browser history, run on every page, or receive your Google password.</p>
            </div>
